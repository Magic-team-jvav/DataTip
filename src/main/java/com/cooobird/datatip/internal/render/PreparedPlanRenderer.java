...
public final class PreparedPlanRenderer {
    ...
    private static void drawPayload(
        ...
        RenderPayload payload = command.payload();
        PreparedImageDraw draw = null;
        if (payload instanceof Visual2DCommandPayload visual) {
            draw = visual.draw();
        } else if (payload instanceof ModelCommandPayload model) {
            draw = model.draw();
        } else if (payload instanceof OverlayCommandPayload overlay) {
            draw = overlay.draw();
        }
        if (draw != null) {
            try {
    ...
    private static void restoreHostTooltipDepth(
        ...
        buffer.vertex(hostTooltipPose, viewportX, viewportY, 0)
            .color(0xFFFFFFFF)
            .endVertex();
        buffer.vertex(hostTooltipPose, viewportX, bottom, 0)
            .color(0xFFFFFFFF)
            .endVertex();
        buffer.vertex(hostTooltipPose, right, bottom, 0)
            .color(0xFFFFFFFF)
            .endVertex();
        buffer.vertex(hostTooltipPose, right, viewportY, 0)
            .color(0xFFFFFFFF)
            .endVertex();
