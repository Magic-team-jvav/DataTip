 ...
 * AtlasContent 解析器�?
 * <p>
 * 负责�?JSON 对象解析�?{@link AtlasContent} 实例�?
 * 支持三种方式指定纹理：直接路径、方�?ID、物�?ID�?
 * </p>
 *
 * <h3>支持�?JSON 格式</h3>
 * <pre>{@code
...
public class AtlasContentParser implements ContentParser {
    ...
    public AtlasContent parse(JsonObject json, ParseContext context) {
        // 获取尺寸（优先级：width/height > size > 默认�?16�?
        int width = context.getInt(json, "width", 16);
        int height = context.getInt(json, "height", width);  // 默认正方�?
