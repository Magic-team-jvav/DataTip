...
DataTip is a JSON-driven custom item tooltip system for Minecraft 1.21.1 and NeoForge 21. Resource packs can combine
text, item icons, textures, rotating blocks and entities, progress bars, charts, animation, and nested layouts without
writing Java code.

Current project version: `1.2.6-neoforge`.
...
DataTip participates in the Minecraft/NeoForge tooltip pipeline instead of replacing it:
...
| Components | `{component:path}` and `{custom_data:path}`                                                                                                                                                       |

Common component paths include `custom_name`, `item_name`, `lore`, `damage`, `max_damage`, `repair_cost`,
`enchantments`, `unbreakable`, `color`, `trim`, and `custom_data`.
...
| `component`                        | Component path that must exist                         |
| `custom_data`                      | Path string that must exist, or object that must match |
...
### Register variables, conditions, and component readers

```java
VariableResolver.registerVariable("my_value", stack -> "42");

ConditionChecker.registerCondition(
    "my_condition",
    (value, stack, player, level) -> Boolean.TRUE.equals(value)
);

ComponentReaderRegistry.register("my_component", reader);
```
